...
function compute_contributions(image::Array{NTuple{PARAM_NUM, Float64}}) where PARAM_NUM
    ...
    padded_image = padarray(image, Fill(Tuple(typemax(Float64) for i in 1:PARAM_NUM), (1,1,1), (1,1,1)))
    ...
    for i in 2:input_size[1]
        for j in 2:input_size[2]
            ...
            # 2d cells
            f = mins_for_tuples((image[i-1, j, k], image[i, j, k]))
            contributions[f] = get(contributions, f, 0) + 1
            f = mins_for_tuples((image[i, j-1, k], image[i, j, k]))
            contributions[f] = get(contributions, f, 0) + 1
            f = mins_for_tuples((image[i, j, k-1], image[i, j, k]))
            contributions[f] = get(contributions, f, 0) + 1
            end
        end
    end
...
const SIZE = 3
img = Array{Tuple{Float64, Float64, Float64}}(undef, SIZE, SIZE, SIZE)
for i in 1:SIZE
    for j in 1:SIZE
        for k in 1:SIZE
            img[i, j, k] = (rand(), rand(), rand())
        end
    end
end
